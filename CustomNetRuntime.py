...
# Assuming CustomAutoencoder and other necessary imports are available
# (e.g., torch, torchvision)

def return_dataloaders(batch_size=32, device='cuda'):
    ...
    # dataset = datasets.MNIST(root='./mnt/fast_storage', train=True, transform=transform, download=True)
    dataset = datasets.STL10(root='./mnt/fast_storage', transform=transform, download=True)
...
def main():
    ...
    # Create dataloaders
    train_loader, val_loader, test_loader = return_dataloaders(batch_size=batch_size, device=device)
    ...
    for epoch in range(epochs):        
        # Ensure data is moved to GPU within the training and validation loop
